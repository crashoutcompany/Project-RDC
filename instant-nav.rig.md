...
- WALLS: OAuth-only auth (no email/password test login); Prisma needs local `.env` or CI Neon branch URLs for build-time prerender; clean runners need `npx playwright install chromium --with-deps` (CI) or `npm run playwright:install` (local); CI needs `NEON_API_KEY`, `NEON_DATABASE`, `NEON_ROLE`, and var `NEON_PROJECT_ID`; e2e also needs auth secret (`AUTH_SECRET` / `BETTER_AUTH_SECRET`, with a CI-only fallback) so `next build`/`next start` can load better-auth
